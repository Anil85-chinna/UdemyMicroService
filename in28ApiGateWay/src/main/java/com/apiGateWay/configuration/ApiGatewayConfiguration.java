...
public class ApiGatewayConfiguration {
	...
	public RouteLocator gatewayRouter(RouteLocatorBuilder builder) {
		return builder.routes()
				...
				// .route(p ->
				// p.path("/currency-conversion/**").uri("lb://currency-conversion"))
				.route(p -> p.path("/currency-conversion/**").uri("lb://currency-conversion-service-1"))
				.route(
						p -> p.path("/currency-conversion-new/**")
								.filters(f -> f.rewritePath("/currency-conversion-new/(?<segment>.*)",
										"/currency-conversion/${segment}"))
								.uri("lb://currency-conversion-service-1")
								)
				.build();
	}
